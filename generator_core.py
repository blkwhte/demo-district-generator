...
def run_generation(config, base_output_dir, status_callback=None, progress_callback=None):
    ...
    edge_case_report = {
        ...
        "Scenario 20 (Section moves to new school Day 3)": [], "Scenario 21 (Section grade unmapped)": [], "Scenario 22 (Inaccurate grades)": [],
        "Scenario 23 (Split Schools)": [], "Scenario 26 (Username Char Limits)": [], "Scenario 28 (Student/Section Grade Mismatch)": [],
        "Scenario 29 (Unsupported Student Grades)": [], "Scenario 30 (SIS ID changes Day 2, reverts Day 3)": [], "Scenario 31 (No Username)": [],
        "Scenario 33 (Student/Teacher match SIS ID)": [], "Scenario 35 (Contact ID changes Day 2)": [], "Scenario 36 (Contact type inconsistent Day 2)": [],
    ...
    for i in range(config["NUM_DISTRICTS"]):
        ...
        for s_idx in range(config["SCHOOLS_PER_DISTRICT"]):
                    ...
                    for period_idx in range(config["SECTIONS_PER_TEACHER_TERM"]):
                        ...
                        if do_3_day:
                            ...
                            if r_sec < 0.02: 
                                ...
                                s_grade = "PK" # Scenario 22: Misclassified Grade
                                edge_case_report["Scenario 22 (Inaccurate grades)"].append(f"Section_id: {sec_id} (Subject: {s_subj}, Classified as PK)")
                            elif r_sec < 0.09:
            ...
            for stu_idx in range(estimated_students):
                ...
                if do_3_day:
                    if random.random() < 0.02: email = email.replace("@", ""); edge_case_report["Scenario 9 (Missing @ in Email)"].append(f"Student_id: {stu_id}")
                    elif random.random() < 0.02: email = f"o'brien.{random.randint(10,99)}@{current_domain}"; edge_case_report["Scenario 8 (Unexpected chars in Email)"].append(f"Student_id: {stu_id}")
                    if random.random() < 0.03: uname = ""; edge_case_report["Scenario 31 (No Username)"].append(f"Student_id: {stu_id}")
                    elif random.random() < 0.02: uname = uname + "superlongusername"*3; edge_case_report["Scenario 26 (Username Char Limits)"].append(f"Student_id: {stu_id}")
        ...
        if do_3_day and len(dist_db["schools"]) > 1:
            ...
            if scen_1_teacher:
                t_copy = scen_1_teacher.copy(); t_copy["School_id"] = school_b; dist_db["teachers"].append(t_copy)
            ...
            if scen_2_student:
                s_copy = scen_2_student.copy(); s_copy["School_id"] = school_b; dist_db["students"].append(s_copy)
            ...
            if scen_3_staff:
                st_copy = scen_3_staff.copy(); st_copy["School_id"] = school_b; dist_db["staff"].append(st_copy)
            ...
            split_school = scen_23_school.copy(); split_school["School_id"] = split_school_id; split_school["School_name"] = scen_23_school["School_name"] + " - Annex"; dist_db["schools"].append(split_school)
